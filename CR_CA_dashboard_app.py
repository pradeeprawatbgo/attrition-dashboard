...
SPREADSHEET_ID_CR = st.secrets["spreadsheet_id_cr"]
SPREADSHEET_ID_CN = st.secrets["spreadsheet_id_cn"]
...
with tabs[0]:
    # st.header("PPT Slide")
    # st.info("Showing a full-page image of a PPT slide. Replace 'static/slide1.png' with your actual image.")
...
with tabs[1]:
    ...
    try:
        ...
        # st.markdown(
        #     f'<iframe src="{pdf_url}&embedded=true" width="100%" height="800px"></iframe>',
...
with tabs[2]:
    ...
    def load_data():
        try:
            df = cr_get_sheet_data(SPREADSHEET_ID_CR, TRACKING_SHEET_RANGE)
            if df is None:
        ...
        with col_save:
            if st.button("Save Comments"):
                try:
                    ...
                    current_df = cr_get_sheet_data(SPREADSHEET_ID_CR, TRACKING_SHEET_RANGE)
                    ...
                    if cr_update_sheet_data(SPREADSHEET_ID_CR, TRACKING_SHEET_RANGE, current_df):
...
with tabs[3]:
    ...
    def load_data1():
        try:
            df1 = ca_get_sheet_data(SPREADSHEET_ID_CN, TRACKING_SHEET_RANGE)
            if df1 is None:
                st.error("Error loading data from Google Sheet")
                return None
            ...
            # If values are greater than 1, assume they are percentages and convert to decimal
            df1['Attrition Probability'] = df1['Attrition Probability'].apply(
                lambda x: x/100 if pd.notnull(x) and x > 1 else x
            )
    ...
    if df1 is not None:
        # st.write("Canada DataFrame columns:", df1.columns.tolist())
        # Add filters at the top: Start Date, End Date, Cost Center, Tenure Bucket
        # Get unique values for filters
        ...
        df1['Attrition Probability'] = df1['Attrition Probability'].fillna(0)  # Fill any NaN values with 0
        ...
        with col6:
            regrettable_count = filtered_df1[filtered_df1['Regrettable Y/N'] == 'Yes'].shape[0]
            st.markdown(f"""
            <div style='text-align: center; padding: 10px; background-color: {metric_colors['regrettable']}; border-radius: 5px;'>
                <p style='color: white; margin: 0;'>Regrettable</p>
                <p style='color: white; margin: 0;'>{regrettable_count}</p>
        ...
        with col2:
            try:
                ...
                if 'Cost Center' in filtered_df1.columns:
                    # Debug information
                    # st.write("Available Attrition Prediction values:", filtered_df1['Attrition Prediction'].unique())
        ...
        table_df1 = filtered_df1.copy()  # Use filtered_df1 instead of df1
        ...
        display_cols = ['SR.No.', 'Report Date','Employee ID','Employee Name','Cost Center','Attrition Prediction','Attrition Probability', 
                    'Risk Level', 'Tenure Bucket (Today Based)','TL Name','Employment Status','Triggers', 'OPS_Comments','Regrettable Y/N','HR Comments (Only In Case Of Non-Regrettable Exits)']
        ...
        with col_save:
            if st.button("Save Comments",key='ca_save_button'):
                try:
                    # Get the current data from Google Sheet
                    current_df1 = ca_get_sheet_data(SPREADSHEET_ID_CN, TRACKING_SHEET_RANGE)
                    if current_df1 is None:
                        st.error("Error loading current data from Google Sheet")
                        st.stop()
                    ...
                    for idx, row in edited_df1.iterrows():
                        ...
                        mask = (current_df1['Employee ID'] == emp_id) & \
                            (current_df1['Report Date'] == date_val)
                        if mask.sum() > 0:
                            ...
                            current_df1.loc[mask, 'OPS_Comments'] = row['OPS_Comments']
                            current_df1.loc[mask, 'Regrettable Y/N'] = row['Regrettable Y/N']

                    # Ensure columns match display_cols (excluding 'SR.No.' and 'Delete')
                    save_cols = [col for col in display_cols if col not in ['SR.No.', 'Delete']]
                    current_df1 = current_df1[save_cols]
                    # st.write("Saving this DataFrame to Google Sheet:", current_df1.head())
                    # st.write("Columns:", current_df1.columns.tolist())

                    # Save back to Google Sheet
                    if ca_update_sheet_data(SPREADSHEET_ID_CN, TRACKING_SHEET_RANGE, current_df1):
                        st.session_state.action_message = "Comments saved successfully!"
                        st.session_state.action_message_type = "success"
                        time.sleep(2)
                        # st.rerun()
