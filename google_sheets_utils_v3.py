...
def cr_get_sheet_data(spreadsheet_id, range_name):
    """Read data from Costa Rica Google Sheet (no column renaming)."""
    try:
        creds = get_google_sheets_credentials()
        if not creds:
            return None
        ...
        st.error(f"Error reading from CR Google Sheet: {str(e)}")
        st.error("Full error details:", str(e.__class__.__name__))
        return None

def ca_get_sheet_data(spreadsheet_id, range_name):
    """Read data from Canada Google Sheet and standardize column names to match CR."""
    column_rename_map = {
        'Ops Comments': 'OPS_comments',
        # Add more mappings as needed to match CR naming
    }
    try:
        creds = get_google_sheets_credentials()
        if not creds:
        ...
        for i, row in enumerate(values):
            if len(row) < num_columns:
                row = row + [''] * (num_columns - len(row))
            padded_values.append(row)
        df1 = pd.DataFrame(padded_values[1:], columns=padded_values[0])
        # st.write("ca col names before renaming",df1.columns.to_list())
        df1 = df1.rename(columns=column_rename_map)
        # st.write("ca col names after renaming",df1.columns.to_list())
        return df1
    except Exception as e:
        st.error(f"Error reading from Canada Google Sheet: {str(e)}")
        st.error("Full error details:", str(e.__class__.__name__))
        return None

def cr_update_sheet_data(spreadsheet_id, range_name, df):
    """Update data in Costa Rica Google Sheet."""
    try:
        creds = get_google_sheets_credentials()
        if not creds:
            return False
        ...
        st.error(f"Error updating CR Google Sheet: {str(e)}")
        st.error("Full error details:", str(e.__class__.__name__))
        return False

def ca_update_sheet_data(spreadsheet_id, range_name, df):
    """Update data in Canada Google Sheet (column names as in DataFrame)."""
    try:
        creds = get_google_sheets_credentials()
        if not creds:
            return False
        ...
        st.error(f"Error updating Canada Google Sheet: {str(e)}")
        st.error("Full error details:", str(e.__class__.__name__))
        return False

def cr_append_sheet_data(spreadsheet_id, range_name, df):
    """Append data to Costa Rica Google Sheet."""
    try:
        creds = get_google_sheets_credentials()
        if not creds:
            return False
        ...
        st.error(f"Error appending to CR Google Sheet: {str(e)}")
        return False

def ca_append_sheet_data(spreadsheet_id, range_name, df):
    """Append data to Canada Google Sheet."""
    try:
        creds = get_google_sheets_credentials()
        if not creds:
            return False
        ...
        st.error(f"Error appending to Canada Google Sheet: {str(e)}")
        return False 